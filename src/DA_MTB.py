...
def DA_MTB(MyData: Data, n_iter: int, DA_impl: str, bool_SIC: bool, seed = 123456789, print_out = False):
    """
    Deferred Acceptance with multiple tie-breaking (my implementation)
    ...
    permut = generate_permutations_MTB(MyData, n_iter, seed, print_out)
...
def generate_permutations_MTB(MyData: Data, n_iter: int, seed = 123456789, print_out = False):
    """
        Will create 'n_iter' tie-breakings of the students

        Returns a list 'permut' containing the tie-breakings

        Each tie-breaking contains one permutation of the students for each school
    """
    ...
    for s in range(MyData.n_schools):
